...
function newFizzBuzz(count, fizzOn, buzzOn) {
  let result = { count, fizz: 0, buzz: 0, fizzBuzz: 0}
  // console.log(count, fizzOn, buzzOn)
  for (let i = 1; i <= count; i += 1) {
    if (i % fizzOn == 0 & i % buzzOn == 0){
      result.fizzBuzz += 1;
    } else if ( i % fizzOn == 0){
      result.fizz += 1;
    } else if (i % buzzOn == 0){
